// ============================================================
// 🏆 TOURNAMENT MANAGER – COMPLETE CLASS WITH ADMIN PANEL
// ============================================================
class TournamentManager {
    ...
    init() {  
        if (this.tournaments.length === 0) {  
            this.tournaments = [  
                { id: 1, name: 'BGMI Livik Clash', map: 'Livik', entryFee: 150, prizePool: 50000, date: '2026-07-10', time: '18:00', maxSlots: 100, registered: 0, status: 'open', roomId: 'ROOM-001' },  
                { id: 2, name: 'Erangel Championship', map: 'Erangel', entryFee: 200, prizePool: 75000, date: '2026-07-15', time: '20:00', maxSlots: 120, registered: 0, status: 'open', roomId: 'ROOM-002' },  
                { id: 3, name: 'Miramar Showdown', map: 'Miramar', entryFee: 100, prizePool: 25000, date: '2026-07-20', time: '16:00', maxSlots: 80, registered: 0, status: 'closed', roomId: 'ROOM-003' }  
            ];  
        ...
        this.setupAdminPanel(); // Admin panel setup
    }  
        ...
        container.innerHTML = this.tournaments.map(t => `  
            <div class="tournament-card" data-id="${t.id}">  
                ...
                <div style="display:flex;justify-content:space-between;color:#999;font-size:0.9rem;margin:8px 0;">  
                    ...
                    <span style="color:${t.status === 'open' ? '#4caf50' : t.status === 'completed' ? '#ffd700' : '#ff2d2d'}">  
                        ${t.status === 'open' ? '🟢 Open' : t.status === 'completed' ? '✅ Completed' : '🔴 Closed'}  
                    </span>  
                </div>  
                ${t.roomId ? `<div style="color:#ffd700;font-size:0.8rem;">🆔 Room: ${t.roomId}</div>` : ''}  
                ${t.winner ? `<div style="color:#ffd700;font-size:0.9rem;">🏆 Winner: ${t.winner}</div>` : ''}  
                <button class="btn btn-primary tournament-register" data-id="${t.id}"   
                        ${t.status === 'closed' || t.status === 'completed' || t.registered >= t.maxSlots ? 'disabled style="opacity:0.5;cursor:not-allowed;"' : ''}>  
                    ${t.status === 'completed' ? 'Completed' : t.status === 'closed' ? 'Registration Closed' : t.registered >= t.maxSlots ? 'Full' : 'Register Now'}  
                </button>  
    ...
    viewRegistrationsHTML(containerId = 'registrationsList') {  
        ...
        for (const [tourId, regs] of Object.entries(this.registrations)) {  
            ...
            if (regs && regs.length > 0) {  
                html += `<div style="margin-bottom:1rem;background:#0d0d0d;padding:12px;border-radius:10px;border-left:3px solid #ffd700;">  
    ...
    setupAdminPanel() {
        ...
        const viewRegBtn = document.getElementById('viewRegistrations');
        if (viewRegBtn) {
            viewRegBtn.addEventListener('click', () => {
                this.viewRegistrationsHTML('registrationsList');
            });
        }
        ...
        if (addForm) {
            addForm.addEventListener('submit', (e) => {
                ...
                const data = {
                    ...
                    maxSlots: parseInt(formData.get('maxSlots')),
        ...
        if (editForm) {
            editForm.addEventListener('submit', (e) => {
                ...
                const data = {
                    ...
                    map: formData.get('editMap'),
                    entryFee: parseInt(formData.get('editEntryFee')),
                    prizePool: parseInt(formData.get('editPrizePool')),
                    date: formData.get('editDate'),
                    time: formData.get('editTime'),
                    maxSlots: parseInt(formData.get('editMaxSlots')),
                    status: formData.get('editStatus'),
                    roomId: formData.get('editRoomId')
                };
                this.editTournament(id, data);
                editForm.reset();
                this.populateAdminSelects();
            });
        }
